...
class Battle extends Component {
  //Load anything before the state loads
  componentDidMount() {
    // const $ = window.$;
    ...
    $(document).ready(function() {
      ...
      function buildVars() {
        ...
        music = {
          ...
          machop:
            "http://66.90.91.26/ost/pokemon-gameboy-sound-collection/viaskmobgb/213-machop.mp3"
        };
        ...
        characters = [
          ...
          },
          {
            ...
            img: {
              default:
                "http://clipart.toonarific.com/data/media/11/pokemon066.gif",
              front: "http://graphics.tppcrpg.net/xy/normal/066F.gif",
              back:
                "http://pokeunlock.com/wp-content/uploads/2015/01/machop-2.gif"
            },
            hp: {
              current: 500,
              total: 500
      ...
      function attackMultiplier(attacker, curAttack) {
        ...
        if (
          gameData[defender].resistance.indexOf(gameData[attacker].type) >= 0
        ) {
          // weakness exists
          curAttack.hp /= 2;
        }
        ...
        if (curAttack.avail.remaining > 0) {
          ...
          if (gameData.enemy.hp.current <= 0) {
            ...
            $(".modal-in header").append(
              '<h1>You Enemy is slain</h1><span class="close">x</span>'
            );
            $(".modal-in section").append(
              "<p>Congratulations! Dare you try again?"
            );
